...
export const loadSelectedProducts = (
  ...
  products: ReadonlyArray<SavedProduct>,
  languages: string[],
): Promise<ReadonlyArray<SelectedProduct>> => {
...
export const searchProducts = (
  ...
  searchString: string,
): Promise<ReadonlyArray<ProductProjection>> =>
  createApiBuilderForProject(config)
    .productProjections()
    .search()
    .get({
...
const hasVariant = <T extends { variant: ProductVariant | undefined }>(
  value: T,
): value is T & { variant: ProductVariant } => !!value.variant;
